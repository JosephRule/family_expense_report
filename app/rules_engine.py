    ...
    def apply_exclusions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for exclusion in exclusions:
            ...
            if 'description_contains' in exclusion:
                mask &= result_df['description'].str.contains(
                    exclusion['description_contains'], case=False, na=False
                )
            
            if 'amount_threshold' in exclusion:
                if exclusion.get('amount_comparison', 'greater') == 'greater':
                    mask &= (result_df['amount'] > exclusion['amount_threshold'])
                else:
                    mask &= (result_df['amount'] < exclusion['amount_threshold'])
